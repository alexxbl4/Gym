...
export function renderCalendar() {
  ...
  let startDay = firstDay === 0 ? 6 : firstDay - 1; els.calDays.innerHTML = '';
  const logsByDate = {};
...
export function renderLibrary() {
  els.librarySearch.value = state.libraryQuery; els.libraryCats.innerHTML = '';
  LIBRARY_CATEGORIES.forEach(cat => {
    const btn = document.createElement('button'); btn.className = `chip ${state.libraryCategory === cat ? 'active' : ''}`;
    btn.dataset.cat = cat; btn.textContent = cat; els.libraryCats.appendChild(btn);
  });
...
export function renderRoutines() {
  const routines = getRoutinesArray(); els.routineList.innerHTML = '';
  if (routines.length === 0) { els.routineList.innerHTML = `<div class="card"><p class="eyebrow">Sin rutinas</p><h3 class="routine-name">Crea tu primera rutina</h3></div>`; return; }
  routines.forEach(routine => {
    const node = els.tplRoutine.content.firstElementChild.cloneNode(true); node.dataset.id = routine.id;
    node.querySelector('.routine-name').textContent = routine.name; node.querySelector('.routine-meta').textContent = `${routine.exercises.length} ejercicios`;
